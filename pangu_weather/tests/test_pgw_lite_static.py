...
class PGWLiteStaticTests(unittest.TestCase):
    ...
    def test_inference_supports_profile_metadata_and_per_channel_dequant(self):
        ...
        self.assertIn('magic == b"\\x1f\\x8b"', source)
        self.assertIn("gzip.open(path, \"rb\")", source)
        self.assertIn('int4_scale_key = key + ".int4_scale"', source)
        self.assertIn("Incomplete INT4 metadata", source)
        ...
        self.assertIn("PANGU_STREAM_WEIGHTS", source)
        self.assertIn("enable_streamed_weight_residency", source)
        self.assertIn('os.environ.setdefault("PANGU_STREAM_WEIGHTS", "0")', source)
        self.assertIn("calibration_affine.npz", source)
        self.assertIn("PANGU_DISABLE_AFFINE_CALIBRATION", source)
        self.assertIn("PANGU_GLOBAL_MEAN_CORRECTION", source)
        self.assertIn("physics_mean_targets.npz", source)
    ...
    def test_profile_model_can_enable_layerwise_forward(self):
        ...
        self.assertIn("def enable_streamed_weight_residency", source)
        self.assertIn("def _run_streamed_module", source)
        self.assertIn("PANGU_STREAM_WEIGHTS", source)
        self.assertIn('"stage"', source)
        self.assertIn('"block"', source)
        self.assertIn("def _direct_patch_recovery", source)
        self.assertIn("def _direct_patch_recovery_scored_only", source)
        ...
        self.assertIn("PANGU_CHUNKED_ATTENTION", source)
        self.assertIn("PANGU_CHUNKED_QKV", source)
        self.assertIn("PANGU_CHUNKED_PROJ", source)
        self.assertIn("chunked_qkv=False", source)
        self.assertIn("x.new_empty(Batch, NumTokens, Channels)", source)
        self.assertIn("x_mlp[:, start:end].copy_", source)
        self.assertNotIn("mlp_chunks", source)
